...
{
  # Waybar が使用するフォントやアイコン・通知送信ツールをインストールする
  home.packages = with pkgs; [
    font-awesome    # アイコンフォント（Waybar のアイコン表示に使用）
    cantarell-fonts # GNOME 標準フォント
    libnotify       # notify-send コマンドを提供する通知ライブラリ
  ];

  # Wayland 向けステータスバー Waybar の設定
  # https://github.com/Alexays/Waybar/wiki/Examples
  # https://github.com/kamlendras/waybar-macos-sequoia
  programs.waybar = {
    enable = true;

    # スタイルシートを同ディレクトリの CSS ファイルから読み込む
    ...
    settings = [
      {
        ...
        mpd = {
          ...
          unknown-tag = "N/A";
          interval = 2;                                # 更新間隔（秒）
          consume-icons = { on = " "; };              # コンシュームモード有効時のアイコン
          random-icons = { on = " "; };               # ランダム再生有効時のアイコン
          repeat-icons = { on = " "; };               # リピート有効時のアイコン
          single-icons = { on = " 1 "; };             # シングル再生有効時のアイコン
          state-icons = {
            paused = "";                              # 一時停止中のアイコン
            playing = "";                             # 再生中のアイコン
          };
  ...
  # 電源メニュースクリプトを ~/.config/waybar/scripts/ に配置する
